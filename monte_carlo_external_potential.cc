...
bool attempt_insertion_external_potential(MyMesh & mesh, int face_type, int rotational_config, UmbrellaWindow & uw, std::mt19937 & eng, ExternalPotential & externalpotential){
	...
	E_before+=externalpotential.energy(mesh, v_to);
	E_before+=externalpotential.energy(mesh, v_from);	
	...
	E_after+=externalpotential.energy(mesh, v_from);
	E_after+=externalpotential.energy(mesh, v_to);
	E_after+=externalpotential.energy(mesh, new_vertex);
...
bool attempt_wedge_insertion_external_potential(MyMesh & mesh, int face_type, int rotational_config, UmbrellaWindow & uw, std::mt19937 & eng, ExternalPotential & externalpotential){
	...
	E_before+=externalpotential.energy(mesh, w1[which]);
	E_before+=externalpotential.energy(mesh, w2[which]);
	E_before+=externalpotential.energy(mesh, w3[which]);		
	...
	E_after+=externalpotential.energy(mesh, w1[which]);
	E_after+=externalpotential.energy(mesh, w2[which]);
	E_after+=externalpotential.energy(mesh, w3[which]);
...
*/
    ...
    E_before = energy(mesh, affected_edges);
	E_before+=uw.bias_potential(n_faces_before);  
	E_before+=externalpotential.energy(mesh, v0);  
	E_before+=externalpotential.energy(mesh, v1);
	E_before+=externalpotential.energy(mesh, v2);		
   
    E_after = halfedge_stretch_energy(mesh, mesh.opposite_halfedge_handle(he), edge_props); 	
	E_after+=uw.bias_potential(n_faces_after);    
	E_after+=externalpotential.energy(mesh, v1, -1, removable_faces[which]);
	E_after+=externalpotential.energy(mesh, v2, -1, removable_faces[which]);	
...
*/
	...
	for (MyMesh::FaceVertexIter fv = mesh.fv_iter(removable_faces[which]); fv.is_valid(); ++fv ){
		E_before+=externalpotential.energy(mesh, *fv);		
	}
	...
	for (MyMesh::FaceVertexIter fv = mesh.fv_iter(removable_faces[which]); fv.is_valid(); ++fv ){
		E_after+=externalpotential.energy(mesh, *fv, -1, removable_faces[which]);		
	}	
...
//	}
	...
	E_before = energy(mesh, affected_edges1) + energy(mesh, affected_edges2);
	//======= wall =====================
	E_before+=externalpotential.energy(mesh, vA);
	E_before+=externalpotential.energy(mesh, vB);
	//===================================
	...
	E_after = energy(mesh, affected_edges);
	//====== wall ===============
	E_after+=externalpotential.energy(mesh, vp);
	//==========================
...
bool attempt_type2_fission_external_potential(MyMesh & mesh, std::mt19937 & eng, ExternalPotential & externalpotential){
	...
	E_before = energy(mesh, affected_edges);
	//======= wall =====================
	E_before+=externalpotential.energy(mesh, v2[which]);
	//===================================		
	...
	//====== wall ===============
	E_after+=externalpotential.energy(mesh, vp);
	E_after+=externalpotential.energy(mesh, v2[which]);	
	//==========================	
...
bool attempt_type1_fission_external_potential(MyMesh & mesh, std::mt19937 & eng, ExternalPotential & externalpotential){
	...
	E_before = energy(mesh, affected_edges);	
	//======= wall =====================
	E_before+=externalpotential.energy(mesh, v1[which]);
	//===================================	
	...
	//====== wall ===============
	E_after+=externalpotential.energy(mesh, vp);
	E_after+=externalpotential.energy(mesh, v1[which]);	
	//==========================	
...
*/
	...
	E_before+=externalpotential.energy(mesh, v1);
	E_before+=externalpotential.energy(mesh, v2);
	E_before+=externalpotential.energy(mesh, v3);
	E_before+=externalpotential.energy(mesh, v4);		
	//===================================	
	...
	E_after = energy(mesh, affected_edges);
	//====== wall ===============
	E_after+=externalpotential.energy(mesh, vn1);
	E_after+=externalpotential.energy(mesh, vn2);	
	//==========================	
...
bool attempt_edge_fission_external_potential(MyMesh & mesh, std::mt19937 & eng, ExternalPotential & externalpotential){
	...
	E_after = energy(mesh, affected_edges);
	//====== wall ===============
	E_after+=externalpotential.energy(mesh, v1);
	E_after+=externalpotential.energy(mesh, v1new);
	E_after+=externalpotential.energy(mesh, v2);
	E_after+=externalpotential.energy(mesh, v2new);			
	//==========================		
